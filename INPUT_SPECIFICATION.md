...
| `scout_keywords` | list of strings | `["scout", "localizer", "survey", "3-plane", "3plane", "aascout", "aahscout", "locator", "scanogram", "plan scan", "planscan", "topogram"]` | Series-description substrings (case-insensitive) identifying localizer/scout sequences to drop during classification. Overrides (does not extend) the default list. |
| `calibration_keywords` | list of strings | `["setter", "prescan", "_cal_", "calibration", "coilsurv"]` | Series-description substrings (case-insensitive) identifying navigator/calibration sequences (e.g., Siemens vNav "setter") eligible for demotion to `DROP_CALIBRATION` via the calibration sequence exclusion pass (Section 2.2). Overrides the default list. |
| `norm_tokens` | list of strings | `["NORM", "PURE", "SCIC", "CLEAR"]` | `ImageType`/`ImageTypeText` tokens identifying the "normalized" member of a vendor reconstruction-variant pair (Siemens NORM, GE PURE, Philips SCIC, and a `CLEAR` token observed on some Philips reconstructions). Used by the NORM/ND anatomical twin resolution pass. Overrides the default list. |
| `expected_anat_count` | mapping of string to integer | `null` | Per-modality expected anatomical count, keyed by `T1W` or `T2W` (case-insensitive on input; normalized to uppercase). When set, a modality whose surviving count exceeds its expectation triggers the two-layer anatomical calibration demotion gate (Section 2.2) before falling back to a `DUPLICATE_MODALITY` warning. `null` (default) preserves the original unconditional `DUPLICATE_MODALITY` warning with no demotion attempted. |
| `registry_mode` | string | `"advisory"` | One of `"strict"` or `"advisory"`. Governs the `exact_volume_counts` guard's behavior for a known series (present in the task registry) whose volume count mismatches its registered `expected_volumes`: `strict` excludes the series; `advisory` retains it and emits a HIGH-severity, user-facing `REGISTRY_VOLUME_MISMATCH` warning instead. |
| `label_freeze_mode` | string | `"frozen"` | One of `"frozen"` or `"re-derive"`. Governs the `no_label_drift` guard's behavior when a registered description re-derives to a label different from the one on record: `frozen` halts with `LabelDriftError`; `re-derive` accepts the fresh derivation and emits a HIGH-severity `LABEL_DRIFT_ADVISORY` warning instead of halting. |
| `rename_detection` | string | `"warn"` | One of `"strict"`, `"warn"`, or `"off"`. Governs the `no_rename_collision` guard's behavior when a new description's acquisition signature matches an existing registry entry under a different description: `strict` halts with `TaskRenameError`; `warn` allows the match through with a HIGH-severity `TASK_RENAME_ADVISORY` warning; `off` skips the signature check entirely. |
...
**Calibration sequence exclusion (fieldmap)**: after the initial classification pass, a two-layer post-classification guard demotes spurious fieldmap candidates to `DROP_CALIBRATION` (silently discarded). The primary layer checks phase-encoding (PE) axis compatibility: an `FMAP_FUNC` series whose PE axis does not match any BOLD series (or an `FMAP_DWI` series whose PE axis does not match any DWI series) is demoted. This is a physics-based guard rooted in the requirement that a fieldmap must share its target's PE axis for distortion correction to be applicable (Jezzard and Balaban, MRM 1995). The secondary layer applies a compound keyword guard: series matching known calibration description keywords (configurable via `calibration_keywords`; default set includes "setter", "prescan", "_cal_", "calibration", "coilsurv") that are single-volume and whose description stem does not match any target modality series are also demoted. If no target modality series exist in the session, both layers are bypassed to avoid false demotion in partial or aborted protocols.

**Calibration sequence exclusion (anatomical)**: a separate, `expected_anat_count`-gated pass runs after the NORM/ND twin resolution pass (below) for T1w and T2w. When a modality's surviving count exceeds its configured expectation, series matching `calibration_keywords` whose description stem is not shared by any non-keyword series of that modality are demoted to `DROP_CALIBRATION` with an `ANAT_CALIBRATION_DEMOTION` warning. Any residual over-count after demotion is reported as a HIGH-severity `DUPLICATE_MODALITY` warning. When `expected_anat_count` is not set (default), an anatomical over-count instead produces the original MEDIUM-severity `DUPLICATE_MODALITY` warning with no demotion attempted. This gate is independent of the fieldmap calibration exclusion above, though both draw from the same `calibration_keywords` list.

**NORM/ND anatomical twin resolution**: within each geometry-paired group of anatomical series, a reconstruction variant carrying one of `norm_tokens` (in `ImageType` or `ImageTypeText`; default `["NORM", "PURE", "SCIC", "CLEAR"]`, covering Siemens NORM, GE PURE, and Philips SCIC/CLEAR reconstructions) is preferred, and its non-normalized twin is demoted to `DROP_ANAT_ND_T1W`/`DROP_ANAT_ND_T2W`. A paired group with no NORM-token member on either side produces a MEDIUM-severity `AMBIGUOUS_CLASSIFICATION` warning instead of a silent guess.
...
<bids_root>/
  ...
  derivatives/
    fmri-bids-recon/  # Conversion reports, manifest.tsv, group conversion summary
...
### 6.1 Group-Level Conversion Summary

On every invocation, after all per-participant processing completes, the pipeline aggregates every `sub-*_ses-*_conversion_report.md` file present in `derivatives/fmri-bids-recon/` into a single plain-text file at `derivatives/fmri-bids-recon/group_conversion_summary_{YYYYMMDD_HHMMSS}.txt` (UTC timestamp at generation time). This step is unconditional (not gated by any config flag) and non-blocking: an exception during generation, or an empty report directory, is logged as a warning and does not alter the pipeline's exit code. Each invocation writes a new, distinctly timestamped file rather than overwriting a prior one, so a study that accrues subjects over multiple pipeline runs retains a group summary per run.

The file has seven fixed-width plain-text sections, in order:

1. **Run Overview**: one row per subject/session with high/medium/low warning counts, excluded-run count, and unclassified-series count, plus a cohort-wide total.
2. **Action Required**: every HIGH-severity review flag, grouped by subject/session, each paired with a plain-language follow-up instruction drawn from a fixed per-`(severity, code)` instruction table (`fmri_bids_recon/group_report.py::FOLLOWUP_INSTRUCTIONS`).
3. **Warnings for Review**: MEDIUM- and LOW-severity review flags, same grouping and follow-up-instruction format as Section 2.
4. **Excluded Runs**: every series excluded by the volume-count guard, with observed vs. expected volume counts, across all subjects/sessions.
5. **Unclassified Series**: every series that matched no classification rule, across all subjects/sessions.
6. **Auto-Registered Tasks**: every task label newly written to the task registry during this run, across all subjects/sessions.
7. **Provenance**: engine version, dcm2niix version, config path, and the list of per-subject report filenames aggregated into this summary.

Subjects and sessions are ordered lexicographically by the `(sub, ses)` string tuple in the Run Overview table and in each per-finding grouping. This is correct for zero-padded BIDS subject labels (`sub-01`, `sub-02`, ..., `sub-10`) but will not sort numerically for unpadded labels once a study exceeds 9 subjects (`sub-10` would sort before `sub-2`); see Known Limitations (Section 7) below.

A review flag code with no matching entry in `FOLLOWUP_INSTRUCTIONS` (e.g., a future warning code added without a corresponding instruction) still renders in Sections 2/3 with a fallback message directing the reviewer to the per-subject report, rather than raising an error.
...
5. **Calibration exclusion validated on Siemens only**: the PE axis validation layer of the calibration sequence exclusion pass is vendor-agnostic by design (rooted in the physical necessity that a fieldmap must share its target's PE axis). However, the guard has been validated only against Siemens vNav setter calibration sequences. Cross-vendor validation against GE and Philips calibration DICOM samples is required to confirm coverage. The default keyword guard layer includes both Siemens-specific terms ("setter", "prescan") and vendor-neutral terms ("_cal_", "calibration", "coilsurv"); additional vendor-specific keywords may be needed as empirical samples become available, and are configurable via `calibration_keywords`.
6. **Group conversion summary subject ordering is lexicographic, not numeric**: the group summary (Section 6.1) sorts subjects by the string `(sub, ses)` tuple. This orders zero-padded BIDS labels correctly but will not sort unpadded numeric labels numerically once a study exceeds 9 subjects (e.g., `sub-10` sorts before `sub-2`). Zero-padded subject IDs avoid this entirely and are the BIDS-recommended convention.
7. **Advisory-mode registry enforcement can produce order-dependent output in shared-registry batch runs**: when `registry_mode="advisory"` (the default) and multiple subjects share a single task registry populated sequentially within the same run, whether an aborted/truncated run is excluded (via within-session tie-breaking, when no registry entry yet exists for that task) or retained with a warning (via advisory-mode registry enforcement, once a registry entry exists) depends on run order. This is inherent to advisory mode's inform-rather-than-block design under shared, sequentially-populated registries; running each subject against an isolated config/registry, or using `registry_mode="strict"`, avoids the asymmetry.
